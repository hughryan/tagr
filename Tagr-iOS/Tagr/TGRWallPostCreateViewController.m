//
//  TGRWallPostCreateViewController.m
//  Tagr
//
...
@interface TGRWallPostCreateViewController ()

@property (nonatomic, assign) NSUInteger maximumCharacterCount;

@end

@implementation TGRWallPostCreateViewController
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        _maximumCharacterCount = [[TGRConfigManager sharedManager] postMaxCharacterCount];
    }
    return self;
}
...
- (IBAction)postPost:(id)sender {
    ...
    PFObject *postObject = [PFObject objectWithClassName:TGRParsePostsClassName];
    postObject[TGRParsePostTextKey] = self.textView.text;
    postObject[TGRParsePostUserKey] = user;
    postObject[TGRParsePostLocationKey] = currentPoint;
    ...
    [postObject saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        ...
        if (succeeded) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:TGRPostCreatedNotification object:nil];
            });
        } else {
